...
int BPFtrace::setup_event_loss()
{
  ...
  if (map.update_elem(const_cast<uint32_t *>(&event_loss_cnt_key_),
                      const_cast<uint64_t *>(&event_loss_cnt_val_)) < 0) {
...
void BPFtrace::handle_event_loss(Output &out)
{
  ...
  auto map = bytecode_.getMap(MapType::EventLossCounter);
  if (map.lookup_elem(const_cast<uint32_t *>(&event_loss_cnt_key_),
                      &current_value) < 0) {
    LOG(ERROR) << "Failed to lookup event loss counter";
  }
  if (current_value) {
...
int BPFtrace::clear_map(const BpfMap &map)
{
  ...
  int err = map.delete_by_keys(keys);
  if (err && err != -ENOENT) {
...
int BPFtrace::zero_map(const BpfMap &map)
{
  ...
  auto keys = map.collect_keys();
  int err = map.zero_out(keys, nvalues);
  if (err && err != -ENOENT) {
    LOG(ERROR) << "failed to look up elem: " << err;
    return -1;
  }
                        ...
                        uint32_t div)
{
  ...
  int err = map.collect_kvs(nvalues, values_by_key);
  if (err) {
    LOG(ERROR) << "failed to look up elem: " << err;
    return -1;
  }
                             ...
                             uint32_t div)
{
  ...
  int err = map.collect_histogram_data(map_info, nvalues, values_by_key);
  if (err) {
    LOG(ERROR) << "failed to look up elem: " << err;
    return -1;
  }
